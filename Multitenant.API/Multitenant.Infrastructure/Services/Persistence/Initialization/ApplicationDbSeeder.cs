namespace Multitenant.Infrastructure.Services.Persistence.Initialization
{
    ...
    internal class ApplicationDbSeeder
    {
        ...
        public ApplicationDbSeeder(MultiTenantInfo currentTenant, RoleManager<UserRole> roleManager, UserManager<User> userManager, CustomSeederRunner seederRunner, ILogger<ApplicationDbSeeder> logger)
        {
        ...
        private async Task SeedRolesAsync(ApplicationDbContext dbContext)
        {
            foreach (string roleName in Roles.DefaultRoles)
            {
                if (await _roleManager.Roles.SingleOrDefaultAsync(r => r.Name == roleName)
                    is not UserRole role)
                {
                    // Create the role
                    _logger.LogInformation("Seeding {role} Role for '{tenantId}' Tenant.", roleName, _currentTenant.Id);
                    role = new UserRole(roleName, $"{roleName} Role for {_currentTenant.Id} Tenant");
        ...
        private async Task AssignPermissionsToRoleAsync(ApplicationDbContext dbContext, IReadOnlyList<Permission> permissions, UserRole role)
        {
            var currentClaims = await _roleManager.GetClaimsAsync(role);
            foreach (var permission in permissions)
            {
                if (!currentClaims.Any(c => c.Type == LocalAppClaims.Permission && c.Value == permission.Name))
                {
